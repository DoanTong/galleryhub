...
import { ADD_PROFILE_IMAGE_ROUTE, HOST, REMOVE_PROFILE_IMAGE_ROUTE, UPDATE_PROFILE_ROUTE } from "@/ultis/constants";
...
const Profile = () => {
  ...
  const validateProfile = () => {
    if(!firstName){
      toast.error("First Name phải được điền vào.")
      return false;
    }
    if(!lastName) {
      toast.error("Last Name phải được điền vào.")
      return false;
    }
    return true;
  };
    ...
    if (validateProfile()){
      try{
        ...
        if(response.status===200 && response.data){
          ...
          toast.success("Profile đã cập nhật thành công.");
          navigate("/chat");
        }
      }catch(error){
        console.log(error)
  ...
  const handleImageChange = async (event) => {
    ...
    if(file){
      ...
      if(response.status===200 && response.data.image){
        ...
        toast.success("Image tải lên thành công.");
      }
    }
  };
  ...
  const handleDeleteImage = async () => {
    try{
      ...
      if(response.status === 200){
        ...
        toast.success("Image đã xóa thành công.");
        setImage(null);
      }
    }catch(error){
      console.log(error);
  ...
  <div className="bg-[#1b1c24] h-[100vh] flex items-center justify-center flex-col gap-10">
    <div className="flex flex-col gap-10 w-[80vw] md:w-max">
      ...
      <div className="grid grid-cols-2">
        ...
        <input type="file" ref={fileInputRef} className="hidden" onChange={handleImageChange} name="profile-name" accept=".png, .jpg, .jpeg, .svg, .webp"/>
        </div>
